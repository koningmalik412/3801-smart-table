...
const Modal = ({ onOpen, onClose, editingProfile }) => {
  ...
  useEffect(() => {
    if (editingProfile) {
      ...
      setHobby(editingProfile.hobby);
    }
  }, [editingProfile]);

  const handleSubmit = async () => {
    const profileData = { name, role, age, hobby };
    if (editingProfile) {
      await fetch(`http://localhost:5000/api/profiles/${editingProfile._id}`, {
        method: "PUT",
        headers: {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center w-full h-screen bg-black bg-opacity-50">
      <div className="relative p-6 w-full max-w-md bg-base rounded-2xl shadow-xl">
        ...
        <div className="mt-6 flex justify-end">
          <button onClick={handleSubmit} className="px-4 py-2 bg-blue text-base rounded-lg">
            Save
          </button>
        </div>
      </div>
          ...
          {profiles.map((profile) => (
            ...
            >
              <div className="flex flex-col items-center pb-10">
                ...
                <p className="text-sm text-brown">Age: {profile.age}</p>
                <p className="text-sm text-brown">Hobby: {profile.hobby}</p>
